...
export async function loadStrategy(projectId){
  const result=await api(`/api/projects/${encodeURIComponent(projectId)}/strategy`,{cache:'no-store'});
  const strategy=result?.data;
  const status=document.querySelector('#strategy-status');
  const resultBox=document.querySelector('#strategy-result');
  if(strategy&&resultBox){resultBox.innerHTML=renderStrategy(strategy);if(status)status.textContent='Analyzed';bindTopicButtons();}
  return strategy;
}

function bindTopicButtons(){
  document.querySelectorAll('[data-use-topic]').forEach(button=>button.addEventListener('click',()=>{
    const input=document.querySelector('#cfg-topic');
    if(input){input.value=button.dataset.useTopic;input.dispatchEvent(new Event('input',{bubbles:true}));input.scrollIntoView({behavior:'smooth',block:'center'});}
  }));
}
...
export function bindStrategy(project){
  ...
  button.addEventListener('click',async()=>{
    ...
    if(message)message.textContent='Reading channel + recent videos and generating strategy…';
    ...
    try{
      ...
      if(message)message.textContent=`Analysis saved. ${Number(result.data.source?.videos?.length||0)} recent videos were analyzed.`;
