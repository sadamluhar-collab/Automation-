...
const COMPAT={openai:{url:'https://api.openai.com/v1/chat/completions'},deepseek:{url:'https://api.deepseek.com/chat/completions'},groq:{url:'https://api.groq.com/openai/v1/chat/completions'},zai:{url:'https://api.z.ai/api/paas/v4/chat/completions'},aihubmix:{url:'https://aihubmix.com/v1/chat/completions'},openrouter:{url:'https://openrouter.ai/api/v1/chat/completions'}};
...
async function freshToken(channelId,credential){
  ...
  if(!expired&&credential.access_token)return decrypt(credential.access_token);
  ...
  if(!response.ok||!data.access_token)throw Object.assign(new Error('YouTube token refresh failed'),{code:response.status===401?'AUTH':'UPSTREAM',status:response.status});
  const expiresAt=new Date(Date.now()+Number(data.expires_in||3600)*1000).toISOString();
  await query('channel_credentials',{method:'PATCH',params:`?channel_id=eq.${encodeURIComponent(channelId)}`,body:{access_token:encrypt(data.access_token),expires_at:expiresAt,updated_at:new Date().toISOString()}});
...
async function collectVideos(token,channel){
  ...
  return (videos?.items||[]).map(v=>({id:v.id,title:text(v.snippet?.title),description:text(v.snippet?.description).slice(0,1000),published_at:v.snippet?.publishedAt||null,duration_seconds:isoDurationSeconds(v.contentDetails?.duration),views:Number(v.statistics?.viewCount||0),likes:Number(v.statistics?.likeCount||0),comments:Number(v.statistics?.commentCount||0),tags:(v.snippet?.tags||[]).slice(0,20)}));
}

async function callCompat(name,key,model,prompt){
  const response=await fetch(COMPAT[name].url,{method:'POST',headers:{Authorization:`Bearer ${key}`,'content-type':'application/json'},body:JSON.stringify({model,messages:[{role:'system',content:'You are the strategy engine for a YouTube automation platform. Return valid JSON only. Do not invent channel facts; use only supplied source data.'},{role:'user',content:prompt}],temperature:0.2})});
  const data=await response.json().catch(()=>({}));
  if(!response.ok)throw Object.assign(new Error(`LLM provider HTTP ${response.status}`),{status:response.status,code:response.status===429?'RATE_LIMIT':'PROVIDER'});
  return data;
}
...
async function callGemini(key,model,prompt){
  ...
  const data=await response.json().catch(()=>({}));
  if(!response.ok)throw Object.assign(new Error(`Gemini HTTP ${response.status}`),{status:response.status,code:response.status===429?'RATE_LIMIT':'PROVIDER'});
  return data;
}

async function generateStrategy(source,config){
  const prompt=JSON.stringify({task:'Analyze this YouTube channel and create a Shorts content strategy for the project.',requirements:{content_type:config.content_type||'YouTube Short',language:config.language||'English',tone:config.tone||'Informative',audience:config.audience||'General',research_depth:config.research_depth||'standard',topic:config.topic||''},output_schema:{channel_analysis:{niche:'string',audience:'string',description_summary:'string',content_pillars:['string'],winning_patterns:['string'],avoid_patterns:['string'],recommended_format:'string'},content_plan:{strategy_summary:'string',topics:[{title:'string',hook:'string',angle:'string',why:'string',estimated_duration_seconds:'number',priority:'high|medium|low'}],cadence:'string',next_best_topic:'string'}},source},null,2);
  let lastError;
