...
async function runPipelineJob(job){
  ...
  const existingState=run.state&&typeof run.state==='object'?run.state:{};
  ...
  const result=await executeStep(step,{job,input:job.input||{},state,checkpoint:job.checkpoint||stepRow.checkpoint||{},providers});
