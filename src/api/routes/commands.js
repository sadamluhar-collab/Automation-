...
export async function execute(req,res){
  try{
    ...
    console.error('command.execute failed',{code:e.code||'COMMAND_FAILED',message:e.message});
    return send(res,e.status||400,{success:false,error:{code:e.code||'COMMAND_FAILED',message:e.message,request_id:req.requestId}});
  }
}

export async function runShort(req,res){
  try{
    const result=await commandCenter.runShort({userId:req.user.id,projectId:req.body?.project_id,prompt:req.body?.prompt,idempotencyKey:req.body?.idempotency_key,source:'command-api'});
    ...
    console.error('command.runShort failed',{code:e.code||'COMMAND_FAILED',message:e.message});
    return send(res,e.status||500,{success:false,error:{code:e.code||'COMMAND_FAILED',message:e.status?e.message:'Short command could not be started',request_id:req.requestId}});
  }
}
