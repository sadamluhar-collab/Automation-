...
import {env,youtubeConfig} from '../config/env.js';

const scope='https://www.googleapis.com/auth/youtube.upload https://www.googleapis.com/auth/youtube.readonly';
const b64=value=>Buffer.from(value).toString('base64url');
const unb64=value=>Buffer.from(value,'base64url').toString('utf8');
const sign=value=>createHmac('sha256',youtubeConfig().WORKER_SECRET).update(value).digest('base64url');
...
export function verifyState(state){
  ...
  let data;try{data=JSON.parse(unb64(payload))}catch{throw new Error('Invalid OAuth state payload')}
...
export async function exchangeCode(code){
  const e=env();youtubeConfig();if(!code)throw new Error('Missing YouTube OAuth code');
  ...
  const data=await r.json().catch(()=>({}));if(!r.ok)throw Object.assign(new Error(data?.error_description||'YouTube OAuth exchange failed'),{code:data?.error||'OAUTH_EXCHANGE',status:r.status,details:data});
  if(!data.access_token)throw Object.assign(new Error('YouTube OAuth did not return an access token'),{code:'OAUTH_EXCHANGE',status:502});
  return data;
}
export async function refreshAccessToken(refreshToken){
  const e=env();youtubeConfig();if(!refreshToken)throw new Error('Missing YouTube refresh token');
  ...
  const data=await r.json().catch(()=>({}));if(!r.ok||!data.access_token)throw Object.assign(new Error(data?.error_description||'YouTube token refresh failed'),{code:data?.error||'TOKEN_REFRESH',status:r.status,details:data});
  return data;
}
