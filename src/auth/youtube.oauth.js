...
export async function exchangeCode(code){
  const e=env();if(!code)throw new Error('Missing YouTube OAuth code');
  ...
  if(!data.access_token)throw new Error('YouTube OAuth did not return an access token');return data;
}

export async function refreshAccessToken(refreshToken){
  const e=env();if(!refreshToken)throw new Error('Missing YouTube refresh token');
