...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(req.method==='GET'&&(u.pathname==='/'||u.pathname.startsWith('/css/')||u.pathname.startsWith('/js/'))){const p=u.pathname==='/'?'/index.html':u.pathname;const data=await readFile(new URL(`../public${p}`,import.meta.url));res.statusCode=200;res.setHeader('content-type',mime(p));res.setHeader('cache-control','no-store, max-age=0');return res.end(data)}
