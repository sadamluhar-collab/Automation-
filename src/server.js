...
import * as projectStrategy from './api/routes/project-strategy.js';
...
import * as artifacts from './api/routes/artifacts.js';
...
import {handleMcp} from './mcp/automation-mcp.js';
...
const server=http.createServer(async(req,res)=>{
  try{
    requestId(req,res,()=>{});limit(req,res,()=>{});if(res.writableEnded)return;
    const u=new URL(req.url,`http://${req.headers.host||'localhost'}`);
    if(u.pathname==='/mcp'||u.pathname.startsWith('/.well-known/')||u.pathname.startsWith('/mcp/.well-known/')||u.pathname==='/register'||u.pathname==='/oauth/authorize'||u.pathname==='/oauth/google/callback'||u.pathname==='/oauth/token')return handleMcp(req,res);
    if(u.pathname==='/health'&&req.method==='GET')return health(req,res);
    if(u.pathname==='/api/automation/readiness'&&req.method==='GET')return automation.readiness(req,res);
    ...
    if(u.pathname==='/api/internet-status'&&req.method==='GET'){const result=await internetStatus();return send(res,result.success?200:503,result)}
    if(req.method==='GET'&&(u.pathname==='/'||u.pathname.startsWith('/css/')||u.pathname.startsWith('/js/')||u.pathname==='/auth/update-password')){const p=u.pathname==='/'?'/index.html':u.pathname;const data=await readFile(new URL(`../public${p}${p.includes('.')?'':'.html'}`,import.meta.url));res.statusCode=200;res.setHeader('content-type',mime(p));res.setHeader('cache-control','no-store, max-age=0');return res.end(data)}
    if(u.pathname==='/api/worker/heartbeat')return workerAuth(req,res,()=>send(res,200,{success:true}));
    if(u.pathname==='/api/browser/commands'&&req.method==='POST'){req.body=await json(req);return auth(req,res,()=>browserControl.enqueue(req,res))}
    if(u.pathname==='/api/browser/commands/claim'&&req.method==='POST'){req.body=await json(req);return workerAuth(req,res,()=>browserControl.claim(req,res))}
    ...
    if(u.pathname.startsWith('/api/projects/')&&u.pathname.endsWith('/strategy/analyze')&&req.method==='POST'){req.params={id:u.pathname.split('/')[3]};req.body=await json(req);return auth(req,res,()=>projectStrategy.analyze(req,res))}
    if(u.pathname.startsWith('/api/projects/')&&u.pathname.endsWith('/strategy')&&req.method==='GET')return auth(req,res,()=>{req.params={id:u.pathname.split('/')[3]};projectStrategy.get(req,res)});
    if(u.pathname.startsWith('/api/projects/')&&u.pathname.endsWith('/run')&&req.method==='POST'){req.params={id:u.pathname.split('/')[3]};req.body=await json(req);return auth(req,res,()=>projects.run(req,res))}
    if(u.pathname.startsWith('/api/projects/')&&u.pathname.includes('/run/')&&req.method==='GET'){const parts=u.pathname.split('/');req.params={id:parts[3],runId:parts[5]};return auth(req,res,()=>projects.runStatus(req,res))}
    ...
    if(u.pathname.startsWith('/api/projects/')&&req.method==='DELETE')return auth(req,res,()=>{req.params={id:u.pathname.split('/')[3]};projects.remove(req,res)});
    if(u.pathname==='/api/jobs'&&req.method==='GET'){req.query=Object.fromEntries(u.searchParams);return auth(req,res,()=>jobs.list(req,res))}
    if(u.pathname==='/api/jobs'&&req.method==='POST'){req.body=await json(req);return auth(req,res,()=>jobs.create(req,res))}
    if(u.pathname.startsWith('/api/jobs/')&&req.method==='POST'){req.params={id:u.pathname.split('/')[3]};req.body=await json(req);return auth(req,res,()=>jobs.action(req,res))}
    if(u.pathname.startsWith('/api/jobs/')&&req.method==='GET')return auth(req,res,()=>{req.params={id:u.pathname.split('/')[3]};jobs.get(req,res)});
    if(u.pathname==='/api/commands/run-short'&&req.method==='POST'){req.body=await json(req);return auth(req,res,()=>commands.runShort(req,res))}
    ...
    if(u.pathname==='/api/schedules'&&req.method==='GET')return auth(req,res,()=>schedules.list(req,res));
    if(u.pathname==='/api/schedules'&&req.method==='POST'){req.body=await json(req);return auth(req,res,()=>schedules.create(req,res))}
    ...
    if(u.pathname==='/api/artifacts'&&req.method==='POST'){req.body=await json(req);return auth(req,res,()=>artifacts.create(req,res))}
    if(u.pathname.startsWith('/api/artifacts/')&&u.pathname.endsWith('/versions')&&req.method==='POST'){req.params={id:u.pathname.split('/')[3]};req.body=await json(req);return auth(req,res,()=>artifacts.version(req,res))}
    if(u.pathname.startsWith('/api/artifacts/')&&u.pathname.endsWith('/status')&&req.method==='POST'){req.params={id:u.pathname.split('/')[3]};req.body=await json(req);return auth(req,res,()=>artifacts.status(req,res))}
    if(u.pathname.startsWith('/api/artifacts/')&&req.method==='GET')return auth(req,res,()=>{req.params={id:u.pathname.split('/')[3]};artifacts.get(req,res)});
    if(u.pathname==='/api/memory'&&req.method==='GET')return auth(req,res,()=>memory.list(req,res));
    if(u.pathname.startsWith('/api/memory/')&&u.pathname.endsWith('/restore')&&req.method==='POST'){req.params={id:u.pathname.split('/')[3]};req.body=await json(req);return auth(req,res,()=>memory.restore(req,res))}
    if(u.pathname.startsWith('/api/memory/')&&req.method==='POST'){req.params={id:u.pathname.split('/')[3]};req.body=await json(req);return auth(req,res,()=>memory.save(req,res))}
    if(u.pathname.startsWith('/api/memory/')&&req.method==='GET')return auth(req,res,()=>{req.params={id:u.pathname.split('/')[3]};memory.get(req,res)});
...
runScheduler().catch(error=>console.error('Scheduler loop stopped',error));
