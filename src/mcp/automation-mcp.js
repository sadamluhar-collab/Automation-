...
const clients=new Map(),codes=new Map(),loginFlows=new Map();
...
const tools=[
{name:'list_channels',description:'List YouTube channels available to the authenticated user.',inputSchema:{type:'object',properties:{}},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'list_projects',description:'List automation projects available to the authenticated user.',inputSchema:{type:'object',properties:{}},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'start_project',description:'Start an automation project pipeline.',inputSchema:{type:'object',properties:{project_id:{type:'string'}},required:['project_id']},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'get_job_status',description:'Get the status of an automation job.',inputSchema:{type:'object',properties:{job_id:{type:'string'}},required:['job_id']},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'get_pipeline_status',description:'Get the status of a pipeline run.',inputSchema:{type:'object',properties:{pipeline_run_id:{type:'string'}},required:['pipeline_run_id']},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'retry_job',description:'Retry a failed automation job.',inputSchema:{type:'object',properties:{job_id:{type:'string'}},required:['job_id']},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'create_schedule',description:'Create a project publishing schedule.',inputSchema:{type:'object',properties:{name:{type:'string'},channel_id:{type:'string'},project_id:{type:'string'},publish_at:{type:'string'},timezone:{type:'string'},schedule_type:{type:'string'},cron_expression:{type:'string'},payload:{type:'object'},enabled:{type:'boolean'}},required:['project_id','publish_at']},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'channel_analytics',description:'Read channel analytics.',inputSchema:{type:'object',properties:{channel_id:{type:'string'}}},securitySchemes:[{type:'oauth2',scopes:['automation']}]},
{name:'list_memory',description:'Read channel/project memory.',inputSchema:{type:'object',properties:{}},securitySchemes:[{type:'oauth2',scopes:['automation']}]}
];
...
function oauthErrorRedirect(flow,error,description){const out=new URL(flow.redirect_uri);out.searchParams.set('error',error);if(description)out.searchParams.set('error_description',description);if(flow.state)out.searchParams.set('state',flow.state);out.searchParams.set('iss',base());return out.toString()}

async function oauthAuthorize(req,res,u){
  const p=u.searchParams;const clientId=p.get('client_id'),redirect=p.get('redirect_uri'),state=p.get('state')||'',scope=p.get('scope')||'openid email automation';const client=clients.get(clientId);
  ...
  const flow=token(),v=verifier();loginFlows.set(flow,{client_id:clientId,redirect_uri:redirect,state,resource:p.get('resource')||`${base()}/mcp`,chatgpt_challenge:p.get('code_challenge'),code_verifier:v,scope,created_at:Date.now()});
  setTimeout(()=>loginFlows.delete(flow),10*60*1000);
  if(req.method==='POST')return completePasswordLogin(req,res,flow);
  const action=`${base()}/oauth/authorize?${new URLSearchParams({flow}).toString()}`;
  return html(res,200,`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>YouTube Automation Login</title><style>body{font-family:system-ui,sans-serif;max-width:420px;margin:60px auto;padding:24px}input,button{box-sizing:border-box;width:100%;padding:12px;margin:7px 0;font-size:16px}button{cursor:pointer}</style></head><body><h2>YouTube Automation</h2><p>Sign in directly with your Supabase account.</p><form method="post" action="${esc(action)}"><input type="hidden" name="client_id" value="${esc(clientId)}"><input type="hidden" name="redirect_uri" value="${esc(redirect)}"><input type="hidden" name="state" value="${esc(state)}"><input type="hidden" name="code_challenge" value="${esc(p.get('code_challenge'))}"><input type="hidden" name="code_challenge_method" value="S256"><input type="hidden" name="response_type" value="code"><input type="hidden" name="resource" value="${esc(p.get('resource')||`${base()}/mcp`)}"><label>Email</label><input name="email" type="email" autocomplete="email" required><label>Password</label><input name="password" type="password" autocomplete="current-password" required><button type="submit">Sign in with Supabase</button></form></body></html>`);
}

async function completePasswordLogin(req,res,flowId){
  const body=await form(req),flow=loginFlows.get(flowId);
  ...
  const email=body.get('email'),password=body.get('password');
  if(!email||!password)return html(res,400,'<!doctype html><html><body><h2>Authorization failed</h2><p>Email and password are required.</p></body></html>');
  const supabaseUrl=process.env.SUPABASE_URL,anon=process.env.SUPABASE_ANON_KEY;
  const r=await fetch(`${supabaseUrl}/auth/v1/token?grant_type=password`,{method:'POST',headers:{apikey:anon,'content-type':'application/json'},body:JSON.stringify({email,password})});
  const d=await r.json().catch(()=>({}));
  if(!r.ok||!d.access_token)return html(res,401,`<!doctype html><html><body><h2>Authorization failed</h2><p>${esc(d.error_description||d.msg||'Supabase sign-in failed. This account may not have a password login enabled.')}</p></body></html>`);
  loginFlows.delete(flowId);
  const outCode=token();codes.set(outCode,{client_id:flow.client_id,redirect_uri:flow.redirect_uri,state:flow.state,resource:flow.resource,challenge:flow.chatgpt_challenge,access_token:d.access_token,refresh_token:d.refresh_token,expires_in:d.expires_in||3600,expires_at:Date.now()+10*60*1000});
  setTimeout(()=>codes.delete(outCode),10*60*1000);
  const out=new URL(flow.redirect_uri);out.searchParams.set('code',outCode);if(flow.state)out.searchParams.set('state',flow.state);out.searchParams.set('iss',base());return res.writeHead(302,{location:out.toString()}).end();
}
...
async function oauthToken(req,res){
  ...
  if(grant==='authorization_code'){
    ...
    codes.delete(p.get('code'));return send(res,200,{access_token:c.access_token,token_type:'Bearer',expires_in:c.expires_in,refresh_token:c.refresh_token,scope:c.scope||'openid email automation'});
  }
...
export async function handleMcp(req,res){
  try{
    ...
    if(u.pathname==='/.well-known/oauth-protected-resource'||u.pathname==='/mcp/.well-known/oauth-protected-resource')return send(res,200,{resource:`${base()}/mcp`,authorization_servers:[base()],bearer_methods_supported:['header'],scopes_supported:['openid','email','automation']});
    ...
    if(u.pathname==='/oauth/authorize'&&(req.method==='GET'||req.method==='POST')){if(req.method==='POST'&&u.searchParams.get('flow'))return completePasswordLogin(req,res,u.searchParams.get('flow'));return oauthAuthorize(req,res,u)}
    if(u.pathname==='/oauth/token'&&req.method==='POST')return oauthToken(req,res);
    if(u.pathname==='/mcp'&&req.method==='GET')return send(res,200,{name:'youtube-automation',version:'1.3.0',protocol:'2025-06-18'});
    const b=await read(req),id=b.id??null;
    if(b.method==='initialize')return send(res,200,{jsonrpc:'2.0',id,result:{protocolVersion:'2025-06-18',capabilities:{tools:{}},serverInfo:{name:'youtube-automation',version:'1.3.0'}}});
    ...
    if(b.method==='tools/call'){try{const d=await tool(req,b.params?.name,b.params?.arguments||{});return send(res,200,{jsonrpc:'2.0',id,result:{content:[{type:'text',text:JSON.stringify(d)}],structuredContent:d}})}catch(e){if(e.status===401)return send(res,200,{jsonrpc:'2.0',id,result:{isError:true,content:[{type:'text',text:'Authentication required'}],_meta:{'mcp/www_authenticate':`Bearer realm="youtube-automation", resource_metadata="${base()}/.well-known/oauth-protected-resource"`}}});throw e}}
